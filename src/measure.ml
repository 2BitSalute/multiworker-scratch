...
module MakeMeasure
  ...
  : Sys_sig.MEASURE = struct

  module Float = Core.Float
  module List = Core.List
  module Telemetry = Telemetry

  module FloatMap = WrappedMap.Make (Core) (struct
      type t = float
  ...
  let print_entry_stats ?record ?print_raw name =
    let print_raw = Option.value print_raw ~default:Stdlib.prerr_endline in
    let record = get_record record in
  ...
  let print_entry_distribution ?record name =
    ...
    | None
    | Some { count = 0.0; _ } ->
      Stdlib.prerr_endline "NO DATA"
    | Some { distribution = None; _ } ->
      Stdlib.prerr_endline "NO DATA (did you forget to call track_distribution?)"
