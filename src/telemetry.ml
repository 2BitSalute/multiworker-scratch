...
module MakeTelemetry
    (Core: Core_sig.COREOPS)
    (Exception : Sys_sig.EXCEPTION)
    (Hh_json : Sys_sig.HH_JSON)
    (String_utils : Sys_sig.STRINGUTILS)
  : Sys_sig.TELEMETRY = struct
  ...
  let quick_gc_stat () : t =
    ...
    (* TODO: what did Jane Street's Stat add that the regular Gc module didn't have? *)
    (* let open Gc.Stat in *)
    create ()
